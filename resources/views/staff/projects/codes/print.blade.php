        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                ...
                <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm text-center">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($codes->count()) }}</p>
                </div>
                <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm text-center">
                    <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Used Codes</p>
                    <p class="text-2xl font-bold text-blue-600">{{ number_format($codes->where('is_used', true)->count()) }}</p>
                </div>
                <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm text-center">
                    <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Unused Codes</p>
                    <p class="text-2xl font-bold text-green-600">{{ number_format($codes->where('is_used', false)->count()) }}</p>
                </div>
            </div>
