        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            ...
            <div class="flex flex-wrap gap-2">
                @if($project->is_active)
                    <a href="{{ route('staff.projects.enumeration.create', $project) }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                        Add New Record
                    </a>
                @endif
                @if($enumerations->count() > 0)
                    <a href="{{ route('staff.projects.enumeration.export', $project) }}?format=csv" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                        Export CSV
                    </a>
                    <a href="{{ route('staff.projects.enumeration.export', $project) }}?format=json" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        Export JSON
                    </a>
                @endif
                                    ...
                                    @foreach($enumerations as $enumeration)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-3">
                                                <a href="{{ route('staff.projects.enumeration.show', $enumeration) }}" class="text-blue-600 hover:text-blue-900">
                                                    View
                                                </a>
                                                <a href="{{ route('staff.projects.enumeration.edit', $enumeration) }}" class="text-indigo-600 hover:text-indigo-900">
                                                    Edit
                                                </a>
                                                <form method="POST" action="{{ route('staff.projects.enumeration.destroy', $enumeration) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this enumeration record?')">
                                                    @csrf
                                                    @method('DELETE')
