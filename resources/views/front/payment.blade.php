    ...
    <body class="flex p-6 lg:p-8 items-center min-h-screen flex-col">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            @if($outstanding > 0 && ($enumerationPayment->projectPayment->payment_type === 'both' || $enumerationPayment->projectPayment->payment_type === 'gateway'))
            ...
            <dialog id="recordPaymentDialog" aria-labelledby="modal-title" class="fixed inset-0 size-auto max-h-none max-w-none overflow-y-auto bg-transparent backdrop:bg-transparent">
                ...
                <div tabindex="0" class="flex min-h-full items-end justify-center p-4 text-center focus:outline-none sm:items-center sm:p-0">
                    <el-dialog-panel class="relative transform overflow-hidden text-left transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in sm:my-8 sm:w-full sm:max-w-2xl data-closed:sm:translate-y-0 data-closed:sm:scale-95">
                        <form id="recordPaymentForm" action="{{ route('front.payments.process', $enumerationPayment) }}" method="POST">
                            @csrf
                                ...
                                <div class="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <div class="relative">
                                            ...
                                            <input type="number" 
                                                ...
                                                max="{{ $outstanding }}"
                                                value="{{ old('amount') }}"
                                                class="block w-full pl-8 pr-3 py-2 border @error('amount') border-red-300 @else border-gray-300 @enderror rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500" 
                                                placeholder="0.00" 
                                                required>
                                        </div>
                                        @error('amount')
                                    ...
                                    <div>
                                        ...
                                        <select id="payment_gateway" 
                                                ...
                                                class="block w-full px-3 py-2 border @error('payment_gateway') border-red-300 @else border-gray-300 @enderror rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500" 
                                                required>
                                            <option value="">Select payment gateway</option>
                                            @foreach($gateways as $gateway)
                                                <option value="{{ $gateway->id }}" {{ old('payment_gateway') == $gateway->id ? 'selected' : '' }}>
                                                    {{ $gateway->name }}
                                                </option>
                                            @endforeach
                                        </select>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                @if($errors->any())
                    const modal = document.getElementById('recordPaymentDialog');
                    if (modal) {
                        modal.showModal();
                    }
                @endif
