        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('projects.update', $project) }}">
                        ...
                        <div class="flex space-x-2">
                            ...
                            @if($project->is_active)
                                <a href="{{ route('projects.deactivate', $project) }}" onclick="event.preventDefault(); document.getElementById('deactivate-form').submit();" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded">
                                    Deactivate Project
                                </a>
                            @else
                                <a href="{{ route('projects.activate', $project) }}" onclick="event.preventDefault(); document.getElementById('activate-form').submit();" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
                                    Activate Project
                                </a>
                            @endif
                        </div>
                    ...
                    @if($project->is_active)
                        <form id="deactivate-form" action="{{ route('projects.deactivate', $project) }}" method="POST" style="display: none;">
                            @csrf
                            @method('PATCH')
                        </form>
                    @else
                        <form id="activate-form" action="{{ route('projects.activate', $project) }}" method="POST" style="display: none;">
                            @csrf
                            @method('PATCH')
                        </form>
                    @endif
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 text-gray-900">
                    ...
                    <div id="add-field-form" class="hidden border border-gray-200 rounded-lg p-4 mb-6">
                        <form method="POST" action="{{ route('projects.addField', $project) }}">
                            @csrf
                                    ...
                                    <div class="flex justify-between items-start">
                                        ...
                                        <div class="flex space-x-2">
                                            ...
                                            @if($field->enumerationData()->count() == 0)
                                                <form method="POST" action="{{ route('fields.delete', $field) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this field?')">
                                                    @csrf
                                                    @method('DELETE')
